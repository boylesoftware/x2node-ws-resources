...
class CollectionResourceHandler extends AbstractResourceHandler {
	...
	GET(call) {
		...
		// execute the DBO in a transaction and return the result promise
				...
				if (refExpr.startsWith(':')) {
					...
					if (hasValue) {
						...
						if (!pred.propDesc.isScalar()) {
							...
							parentGroupIds.add(groupId);
							const nestedGroup = this._parseFilterParams(
								propDesc.nestedProperties, valExpr, ':and',
	...
	POST(call) {
		...
		const parentCountDBO = this._buildParentRecordCountDBO(
			call, record, uplinkQueryParams);

		// validate immediate uplink value
		if (!this._validateImmediateUplinkRef(call, record))
			return ws.createResponse(400).setEntity({
		...
		if (parentCountDBO)
			...
			txPhases.beforeAction = tx => parentCountDBO.execute(
		...
		if ((responseType === undefined) || (responseType === 'record'))
			txPhases.afterAction = (tx, recordId) => recordFetchDBO.execute(
				tx, call.actor, {
					id: recordId
				});

		// execute the transaction
		let responsePromise = this._executeTransaction(txPhases);
...
<html lang="en">
  ...
  <body>Location: <a href="${location}">${this._recordTypeName}</a></body>
<html>
`, 'utf8'), 'text/html; charset=UTF-8');
		...
		default: // re-read the new record
			responsePromise = responsePromise.then(
				result => {
					const location = call.requestUrl.pathname + '/' +
