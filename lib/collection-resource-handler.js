	...
	GET(call) {
		...
		responsePromise = responsePromise.then(
			() => {
				...
				// process conditional request, calculate etag
				txPhases.push(
					(_, txCtx) => this._processConditionalRequest(
						txCtx, searchDBO.involvedRecordTypeNames));

				// custom "before" hook
				if ((typeof this.beforeSearch) === 'function')
					txPhases.push(
						(_, txCtx) => this.beforeSearch(txCtx));
				...
				if ((typeof this.afterSearch) === 'function')
					txPhases.push(
						(_, txCtx, result) => this.afterSearch(txCtx, result));
	...
	POST(call) {
		...
		responsePromise = responsePromise.then(
			() => {
				...
				// build DBO for fetching new record
				// TODO: can be cached
				...
				if ((typeof this.beforeCreate) === 'function')
					txPhases.push(
						(_, txCtx) => this.beforeCreate(
							txCtx, txCtx.recordTmpl));

				// create insert DBO and execute the main action
				txPhases.push(
					tx => this._dboFactory
						.buildInsert(this._recordTypeName, txCtx.recordTmpl)
				...
				if ((typeof this.afterCreate) === 'function')
					txPhases.push(
						(_, txCtx, record) => this.afterCreate(txCtx, record));
...
<html>`, 'utf8'), 'text/html; charset=UTF-8');
	...
	/**
	 * Build fetch DBO that gets the parent record id based on the uplink URI
	 ...
	 */
	_buildParentRecordDBO(call, record, queryParams) {
	 ...
	 * context.
	 * @param {Set.<string>} recordTypeNames Names of record types, records of
	 * which contribute to the response besides the main record.
	 * @returns {Promise} Transaction phase completion promise.
	 */
	_processConditionalRequest(txCtx, recordTypeNames) {

		// check if the app uses record collections monitor
		const rcMonitor = this._dboFactory.recordCollectionsMonitor;
		if (!rcMonitor)
			return;

		// calculate the ETag and modification timestamp
		return rcMonitor.getCollectionsVersion(
			txCtx.transaction, recordTypeNames, 'shared').then(
				versionInfo => {
					...
					const etag =
						...
						':' + String(
							txCtx.call.actor ? txCtx.call.actor.id : '*') +
						':' + versionInfo.version + '"';
