...
class TransactionContext {
	...
	/**
	 * Convenience shortcut for building and executing an update DBO. The update
	 * also includes post-patch record validation using the "onUpdate"
	 * validation set.
	 *
	 * @see [buildUpdate()]{@link module:x2node-dbos~DBOFactory#buildUpdate}
	 *
	 ...
	 * this value instead of the update DBO result (which is lost in that case).
	 * @returns {(Promise.<module:x2node-dbos~UpdateDBO~Result>|*)} Promise of
	 * either the update result object or the pass through object. If the update
	 * didn't pass the post-patch validation, the promise is rejected with
	 * [ValidationErrors]{@link module:x2node-validators~ValidationErrors}.
	 */
	updateAndValidate(recordTypeName, patchSpec, filterSpec, passThrough) {
		...
		let resultPromise = this._dboFactory.buildUpdate(
			recordTypeName, patches.build(
				this._recordTypes, recordTypeName, patchSpec), filterSpec)
			.execute(this._tx, this._call.actor, patchedRecord => {
				...
				const errors = validators.normalizeRecord(
					...
					recordTypeName,
					patchedRecord,
					this._call.httpRequest.headers['Accept-Language'],
					'onUpdate'
				);
				if (errors)
