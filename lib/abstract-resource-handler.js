...
class AbstractResourceHandler {
	 ...
	 * @param {module:x2node-ws-resources~TransactionContext} txCtx Transaction
	 * context.
	 * @param {string} [recordTypeName] Main record type name.
	 * @param {(number|string)} [recordId] Main record id.
	 * @param {Set.<string>} [otherRecordTypeNames] Names of record types,
	 * records of which contribute to the response besides the main record.
	 * @returns {Promise} Transaction phase completion promise.
	 */
	_processConditionalRequest(
		txCtx, recordTypeName, recordId, otherRecordTypeNames) {

		// TODO: individual record
		...
		return rcMonitor.getCollectionsVersion(
			txCtx.transaction, otherRecordTypeNames, 'shared').then(
				versionInfo => {
	 ...
	 * present on the transaction context. The values are set by the
	 * <code>_processConditionalRequest</code> method.
	 *
	 * @protected
	 * @param {module:x2node-ws-resources~TransactionContext} txCtx Transaction
	 * context.
