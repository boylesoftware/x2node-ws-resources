...
class AbstractResourceHandler {
	 ...
	 * @returns {Promise} The promise that, if successfully resolved, allows the
	 * transaction to proceed to the main action execution. If the promise is
	 * rejected, the main action is never executed, the transaction is rolled
	 * back (unless explicitely committed by the hook), and the whole handler
	 * call is rejected with the same rejection object.
	 */
	/**
