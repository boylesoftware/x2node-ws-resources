...
class AbstractResourceHandler {
	...
	constructor(ds, dboFactory, rsrcPath, options) {
		 ...
		 *
		 * @member {module:x2node-dbos~TxFactory} module:x2node-ws-resources~AbstractResourceHandler#_txFactory
		 */
		this._txFactory = dboFactory.createTxFactory(ds);
	...
	_createTransactionContext(call) {
		...
		return new TransactionContext(call, this._dboFactory, this._log);
	}
	 ...
	 *
	 * @protected
	 * @param {string} hookName Hook name (e.g. 'beforeRead', 'afterUpdate',
	 * etc.).
	 * @param {...*} hookArgs Hook arguments. First argument is always
	 * transaction context.
	 * @returns {*} Hook result.
	 */
	_callHook(hookName, ...hookArgs) {

		const txCtx = (
			hookName.startsWith('complete') ? hookArgs[1] : hookArgs[0]);

		if (!txCtx.log.enabled)
			return this[hookName].apply(this, hookArgs);

		txCtx.log(`calling ${hookName} hook`);
		const hookResult = this[hookName].apply(this, hookArgs);
		if (hookResult && ((typeof hookResult.then) === 'function'))
			return hookResult.then(res => {
				txCtx.log(`finished ${hookName} hook`);
				return res;
			});
