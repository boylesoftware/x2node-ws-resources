...
class IndividualResourceHandler extends AbstractResourceHandler {
	...
	GET(call) {
		...
		if ((typeof this.prepareRead) === 'function')
			responsePromise = responsePromise.then(
				() => Promise.resolve(this.prepareRead(call, txCtx))
			);
		...
		responsePromise = responsePromise.then(
			() => {
				...
				if ((typeof this.beforeRead) === 'function')
					txPhases.push(
						(tx, txCtx) => this.beforeRead(tx, txCtx));
				...
				if ((typeof this.afterRead) === 'function')
					txPhases.push(
						(tx, txCtx, record) => this.afterRead(
							tx, txCtx, record));
		...
		if ((typeof this.completeRead) === 'function')
			responsePromise = responsePromise.then(
				record => Promise.resolve(
					this.completeRead(undefined, call, txCtx, record)),
				err => Promise.reject(
					this.completeRecord(err, call, txCtx, undefined))
			);
	...
	PATCH(call) {
		...
		if ((typeof this.prepareUpdate) === 'function')
			responsePromise = responsePromise.then(
				() => Promise.resolve(this.prepareUpdate(call, txCtx))
			);
		...
		responsePromise = responsePromise.then(
			() => {
				...
				if ((typeof this.beforeUpdate) === 'function')
					txPhases.push(
						(tx, txCtx) => this.beforeUpdate(tx, txCtx));
				...
				if ((typeof this.afterUpdate) === 'function')
					txPhases.push(
						(tx, txCtx, record) => this.afterUpdate(
							tx, txCtx, record));
		...
		if ((typeof this.completeUpdate) === 'function')
			responsePromise = responsePromise.then(
				record => Promise.resolve(
					this.completeUpdate(undefined, call, txCtx, record)),
				err => Promise.reject(
					this.completeUpdate(err, call, txCtx, undefined))
			);
	...
	DELETE(call) {
		...
		if ((typeof this.prepareDelete) === 'function')
			responsePromise = responsePromise.then(
				() => Promise.resolve(this.prepareDelete(call, txCtx))
			);
		...
		responsePromise = responsePromise.then(
			() => {
				...
				if ((typeof this.beforeDelete) === 'function')
					txPhases.push(
						(tx, txCtx) => this.beforeDelete(tx, txCtx));
		...
		if ((typeof this.completeDelete) === 'function')
			responsePromise = responsePromise.then(
				() => Promise.resolve(
					this.completeDelete(undefined, call, txCtx)),
				err => Promise.reject(
					this.completeDelete(err, call, txCtx))
			);
